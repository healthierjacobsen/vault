CREATE OR REPLACE FUNCTION sp_latest_responses(_program_id integer, _set_id integer, _org_type_id integer,
...
BEGIN
    ...
    IF org_graded THEN
        ...
        FROM (SELECT o.pid,
                     ...
                     ROW_NUMBER() OVER ( PARTITION BY r.organization_id, r.criterion_id
                         ORDER BY r.organization_id, r.created_at DESC) AS row_num
    ...
    RAISE NOTICE 'Starting Current Value Crosstab';
    PERFORM public.sp_responses_value_ct(_set_id, 'temp_latest_responses');
    RAISE NOTICE 'Starting Current Date Crosstab';
    PERFORM public.sp_responses_date_ct(_set_id, 'temp_latest_responses');
    RAISE NOTICE 'Building Current Responses Crosstab Table';
    ...
    RAISE NOTICE 'Starting Current Module Baselines %', before_date;
    PERFORM public.sp_module_baseline(_set_id, 'temp_latest_responses', before_date, _org_type_id);

END;
$$;

ALTER FUNCTION sp_latest_responses(integer, integer, integer, date) OWNER TO main;
