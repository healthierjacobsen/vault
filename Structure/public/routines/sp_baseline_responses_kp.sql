CREATE OR REPLACE FUNCTION sp_baseline_responses_kp(_program_id integer, _set_id integer, _org_type_id integer,
    ...
    IF org_graded THEN
        ...
        FROM (SELECT o.pid,
                     ...
                     ROW_NUMBER() OVER ( PARTITION BY r.organization_id, r.criterion_id
                         ORDER BY r.organization_id, r.created_at ASC
                         --                              CASE WHEN EXTRACT(EPOCH FROM r.created_at) - EXTRACT(EPOCH FROM date('2018-01-01')) < 0 THEN
--                                       ABS(EXTRACT(EPOCH FROM r.created_at) - EXTRACT(EPOCH FROM date('2018-01-01')))
--                                   WHEN EXTRACT(EPOCH FROM date('2018-01-01')) - EXTRACT(EPOCH FROM r.created_at) > 0 THEN
--                                           EXTRACT(EPOCH FROM r.created_at) - EXTRACT(EPOCH FROM date('2018-01-01'))
--                                  END ASC
                         )                                                                         AS row_num
...
--                                  END ASC
    ...
    RAISE NOTICE 'Starting Module Baselines';
    PERFORM public.sp_module_baseline(_set_id, 'temp_baseline_responses', before_date, _org_type_id);

END;
$$;

ALTER FUNCTION sp_baseline_responses_kp(integer, integer, integer, date) OWNER TO main;

GRANT EXECUTE ON FUNCTION sp_baseline_responses_kp(integer, integer, integer, date) TO chartio;
