CREATE OR REPLACE FUNCTION sp_baseline_responses(_program_id integer, _set_id integer, _org_type_id integer,
...
--                      ci.handle,
--                      rv.value,
--                      r.created_at, ROW_NUMBER() OVER ( PARTITION BY r.organization_id, r.criterion_id
...
--                        JOIN live_data.criterion_instances ci ON r.criterion_id = ci.criterion_id AND _set_id = ci.set_id AND ci.deleted_at IS NULL
...
--     END IF;
    ...
    CREATE TEMPORARY TABLE temp_baseline_responses AS
    WITH o AS (
        SELECT o.id, o.pid, o.name, o.grade_level_high, o.grade_level_low, o.grade_level_ids
        FROM live_data.organizations o
        WHERE o.organization_type_id = _org_type_id
    ...
    FROM (
             SELECT *,
                    ROW_NUMBER()
                    OVER (PARTITION BY org_id, criterion_id ORDER BY org_id, criterion_id, created_at DESC) AS row_num
             FROM (
                      ...
                      FROM (
                               ...
                               FROM live_data.responses r
                                        ...
                                        JOIN o ON o.id = r.organization_id
                               WHERE r.created_at > (SELECT created_at FROM live_data.sets WHERE id = _set_id)
                                 AND r.created_at < _current_date
                           ) z
                      WHERE row_num = 1
                      UNION
                      SELECT o.id AS org_id, b.criterion_id, rv.value, b.created_at
                      FROM o
                               JOIN public.baseline_responses b
                                    ON b.organization_id = o.id
                               JOIN live_data.response_values rv ON rv.id = b.response_value_id
                      WHERE b.created_at > (SELECT created_at FROM live_data.sets WHERE id = _set_id)
                        AND b.created_at < _current_date
                  ) c
         ) b
             JOIN o ON o.id = b.org_id
             JOIN live_data.criterion_instances ci ON ci.criterion_id = b.criterion_id
    WHERE b.row_num = 1
      AND ci.set_id = _set_id
...
ALTER FUNCTION sp_baseline_responses(integer, integer, integer, date) OWNER TO main;

GRANT EXECUTE ON FUNCTION sp_baseline_responses(integer, integer, integer, date) TO chartio;
